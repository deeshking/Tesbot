      ...
      case 'aspan': case 'ptl': case 'ptlvid':{
        if (!isPremium) return reply(mess.OnlyPrem)
          fetchText('http://sansekai.my.id/sansekai.txt').then((data) => {
            ...
            var wifegerakx = wifegerak[Math.floor(Math.random() * wifegerak.length)]
            queen.sendFileFromUrl(from, `http://sansekai.my.id/ptl_repost/${wifegerakx}`, 'Follow IG: https://www.instagram.com/ptl_repost untuk mendapatkan asupan lebih banyak.', msg)
          })
      }
      break
      case 'spamcall2':{
...
▹ ${prefix}wallgravity text1 text2
 `
                                sendButMessage(from, wibu, `created follow @king_dishuu`, [
                                {
...
`
      ...
      if (fs.existsSync('./lib/vote/' + from + '.json') && fs.existsSync('./lib/' + from + '.json')) {
        deleteVote(from);
        reply("Success Menghapus sesi Voting Di Grup Ini");
      } else {
...
▹ ${prefix}bokep25
 `
    ...
    sendButMessage(from, menu, `created follow @king_dishuu`, [
    {
      buttonId: noprefix ? "menu" : "!"+"menu",
      buttonText: {
        displayText: `⬡ BACK TO MENU `,
...
▹ ${prefix}jadwalsholat`
   ...
   case 'command':
   list = []
   listmenu = [`groupmenu`,`grupmenu`,`photoxy`,`ephoto`,`randomimage`,`wibu2`,`storymenu`,`porno`,`randomtext`,`islammenu`,`wibumenu`,`stickermenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`infomenu`,`othermenu`,`owner`,`queengroup`,`sewabot`]
   listmenuu = [`Menu Group`,`Menu Group2 (button menu)`,`Photo Oky`,`Ephoto Menu`,`Random Image`,`Nsfw Anime`,`Movie&Story`,`18+ Menu`,`RandomText`,`Islam Menu`,`Wibu Menu`,`Sticker Menu`,`Owner Command`,`Game Menu`,`For Fun Menu`,`Downloader`,`Info Menu`,`Menu Lainnya`,`OwnerBot`,`Official Group`,`Rent this Bot`]
   nombor = 1
   startnum = 0
   for (let x of listmenu) {
...
case 'addcmd': 
case 'setcmd':
if (!isPremium) return reply(`You are not a premium user, send an order *${prefix}buypremium* to buy premium`)
  if (isQuotedSticker) {
